...
# During he 2016 US presidential election then candidate Donald J. Trump 
# used his tweeter account as a way to communicate with potential voters. 
# On August 6, 2016 Todd Vaziri tweeted about Trump that "Every non-hyperbolic 
# tweet is from iPhone (his staff). Every hyperbolic tweet is from Android 
# (from him)." Data scientist David Robison conducted an analysis to determine 
# if data supported this assertion. Here we go through David's analysis to 
# learn some of the basics of text mining. To learn more about text mining 
# in R we recommend this book.
...
# at http://www.trumptwitterarchive.com. We
...
# between 6 and 8 AM. There seems to be a clear different in these 
# patterns. We will therefore assume that two different entities are 
# using these two devices. Now we will study how their tweets differ. 
...
# refers to the units that we are considering to be a data point. The 
# most common token will be words, but they can also be single characters, 
...
# characters important to twitter such as # and @. A token in twitter 
# is not the same as in regular english. For this reason instead of 
# using the default, words, we define a regex that captures twitter 
# character. The pattern appears complex but all we are defining is a 
# pattern that starts with @, # or neither and is followed by any 
# combination of letter or digits:
...
# It is not surprising that these are the top words. The top words 
# are not informative. The tidytext package has database of these 
# commonly used words, referred to as stop words, in text mining:
...
# Some exploration of the resulting words (not show here) reveals a 
...
# come from a quote and they start with '. We want to remove the ' when 
# it's at the start of a word so we will use str_replace. We add these 
# two lines to the code above to generate are final table:
...
# a summary statistic useful for quantifying these differences. We each device 
# and a given word, let's call it y, we compute the odds or the ratio between the 
...
# Hyperbolic is a hard sentiment to extract from words as it relies on interpreting 
# phrases. However, words can be associated to more basic sentiment such as as anger, 
# fear, joy and surprise. In the next section we demonstrate basic sentiment analysis.
...
# The bing lexicon divides words into positive and negative. We can see this using 
# the tidytext function get_sentiments:
...
# words associated with a sentiment. Here are 10 random words extracted from the tweets:
...
# this somewhat complex since each tweet will have several sentiments attached to it, 
# one for each word appearing in the lexicon. For illustrative purposes we will perform 
# a much simpler analysis: we will count and compare the frequencies of each sentiment 
...
# three sentiments are disgust, anger, and negative! But are they statistically 
...
# confidence interval. We will add the two values we need to form a two-by-two table 
# and the odd rat
...
# source, whic is in agreement with the original claim about hyperbolic tweets.
