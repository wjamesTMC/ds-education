...
# Alternative to the above: Collect data and save the data file
...
# If we examine the object text we notice that it is 
# a character vector with an entry for each page. 
# So we keep the page we want using the following code:
  
raw_data_research_funding_rates <- txt[2]

# The steps above can actually be skipped because we include the 
# raw data in the dslabs package as well:
...
# including the table rows, is separated by the symbol 
# for newline: \n.

# We can therefore can create a list with the lines of 
# the text as elements:
  
tab <- str_split(raw_data_research_funding_rates, "\n")

# Because we start off with just one element in the string, 
# we end up with a list with just one entry:
...
# we see that the information for the column names is the 
# third and forth entires:
  
the_names_1 <- tab[3]
the_names_2 <- tab[4]

# In the table, the column information is spread across two 
# lines. We want to create one vector with one name for each 
...
# We want to remove the leading space and everything following 
# the comma. We can use regex for the latter. Then we can obtain 
# the elements by splitting using the space. We want to split 
# only when there are 2 or more spaces to avoid splitting success 
...
# Here we want to trim the leading space and then split by space 
# as we did for the first line:
